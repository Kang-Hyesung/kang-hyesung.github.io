...
### `UseWhen`의 두 가지 핵심 요소: 조건과 분기

`UseWhen` 메서드는 근본적으로 두 개의 파라미터를 받아 동작합니다. "만약 **(이 조건)이 참이면, (이 분기)를 실행하라**"는 명확한 구조를 가집니다.

#### 1\. 첫 번째 파라미터: 조건 (Predicate)

이 파라미터는 `HttpContext`를 입력으로 받아 `bool`(`true` 또는 `false`) 값을 반환하는 함수입니다. 들어오는 모든 요청은 이 조건 함수를 거치게 되며, 여기서 반환되는 값에 따라 다음에 실행될 코드가 결정됩니다.

  * **`true` 반환 시**: 두 번째 파라미터인 '미들웨어 분기'가 실행됩니다.
  * **`false` 반환 시**: '미들웨어 분기'를 건너뛰고, 파이프라인의 다음 미들웨어로 바로 진행합니다.

#### 2\. 두 번째 파라미터: 미들웨어 분기 (Branch)

이 파라미터는 첫 번째 '조건'이 `true`를 반환했을 때만 실행되는 독립적인 미들웨어 파이프라인을 정의하는 코드 블록입니다. 이 분기 안에는 하나 이상의 미들웨어를 `Use`, `Run` 등으로 자유롭게 구성할 수 있습니다. 이 분기된 파이프라인의 실행이 끝나면, 제어 흐름은 다시 원래의 주 파이프라인(Main Chain)으로 돌아갑니다.
