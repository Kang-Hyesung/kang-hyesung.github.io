...
Java 개발자가 `int c = a + b;`라는 코드를 작성할 때, 런타임 환경에서는 단순히 두 변수의 값을 더해 할당하는 것 이상의 작업이 수행된다. 이 과정에서 발생하는 **임시 결과(Temporary Result)** 와 **타입 프로모션(Type Promotion)** 의 원리를 이해하는 것은 정확한 데이터 타입 선정과 잠재적인 버그 방지를 위해 필수적이다.
