...
graph LR
    ...
    style B fill:#991b1b,stroke:#94a3b8,stroke-width:2px,color:#f8fafc
...
>
> 멀티스레드 환경에서는 한 스레드가 공유 변수 값을 변경하더라도, 다른 스레드가 그 변경을 즉시 본다고 보장할 수 없다. Java Memory Model 관점에서는 각 스레드가 공유 변수의 사본을 작업 메모리에 두고 사용할 수 있으며, 동기화 규칙이 없으면 오래된 값을 계속 읽는 상황이 발생할 수 있다.
> 메인 스레드가 `exitFlag = true`로 값을 변경하더라도, 워커 스레드가 여전히 이전 값(`false`)을 보고 있다면 워커 스레드는 영원히 종료되지 않는 무한 루프(Infinite Loop)에 빠질 수 있다. 단 1 나노초의 오차도 없이 즉각적으로 값이 반영되지 않으며 동기화 지연이 발생한다.
>
> 이를 해결하려면 변수 선언 시 `volatile` 키워드를 사용하여, 해당 변수의 읽기/쓰기에 대한 스레드 간 가시성을 보장해야 한다. (`private static volatile boolean exitFlag = false;`) `volatile`은 CPU 캐시를 사용하지 않게 만드는 키워드가 아니라, JVM이 해당 변수에 대해 JMM의 가시성 규칙을 지키도록 만드는 동기화 수단이다.
{: .prompt-info }
...
<div>
메모리 가시성(Memory Visibility) 문제입니다. 한 스레드가 공유 변수 값을 변경해도 다른 스레드가 그 변경을 즉시 본다고 보장되지 않기 때문에 발생합니다. Java Memory Model 관점에서는 스레드가 오래된 작업 메모리 사본을 계속 읽을 수 있으므로, `volatile` 같은 동기화 수단으로 가시성을 보장해야 합니다.
</div>
</details>
