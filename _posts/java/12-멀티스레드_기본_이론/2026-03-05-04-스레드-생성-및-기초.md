...
flowchart TD
    subgraph Main["Main Thread"]
        A["new Thread(Runnable)<br/>스레드 객체 생성"]
        B["start()<br/>새 스레드 시작 요청"]
        G["Main Thread<br/>다음 코드 계속 실행"]
        A --> B
    end

    subgraph Runtime["JVM / OS"]
        C0{"start() 이후<br/>실행 흐름 분기"}
        C["start0()<br/>Native 메서드 호출"]
        D["OS에 실제 스레드<br/>생성 요청"]
        E["새 호출 스택<br/>할당"]
        C0 --> C
        C --> D
        D --> E
    ...
    classDef main fill:#14532d,stroke:#22c55e,stroke-width:2px,color:#f8fafc
    classDef runtime fill:#854d0e,stroke:#f59e0b,stroke-width:2px,color:#f8fafc
    classDef branch fill:#7c2d12,stroke:#fb923c,stroke-width:2px,color:#f8fafc
    classDef worker fill:#075985,stroke:#38bdf8,stroke-width:2px,color:#f8fafc
    classDef neutral fill:#374151,stroke:#94a3b8,stroke-width:2px,color:#f8fafc
