...
sequenceDiagram
    ...
    participant Lock as lock 객체<br/>Monitor Lock
    participant T2 as Thread 2

    T1->>Lock: 락 요청
    Lock-->>T1: 락 획득
    Note over T1: synchronized 블록 실행

    T2->>Lock: 락 요청
    Lock-->>T2: 이미 Thread 1이 사용 중
    Note over T2: Entry Set에서 대기

    T1->>Lock: 작업 완료 후 락 반납
    Lock-->>T2: 락 획득
    Note over T2: synchronized 블록 실행
```
