...
graph LR
    ...
    style D fill:#166534,stroke:#94a3b8,stroke-width:2px,color:#f8fafc
...
class FileIOThread extends Thread {
    /**
     * volatile: 여러 스레드가 이 변수를 참조할 때 값 변경의 가시성을 보장한다.
     * 이를 통해 메인 스레드에서 값을 변경했을 때 워커 스레드가 오래된 값을 계속 읽지 않고
     * 종료 요청을 감지할 수 있다.
     */
