...
```mermaid
flowchart TD
  A["NEW<br/>스레드 객체만 생성"] -->|"start()"| B["RUNNABLE<br/>실행 가능 상태"]
  B -->|"run() 종료"| F["TERMINATED<br/>실행 종료"]

  B -->|"synchronized 락 대기"| C["BLOCKED<br/>락 획득 대기"]
  C -->|"락 획득"| B

  B -->|"wait(), join()"| D["WAITING<br/>다른 스레드의 신호 대기"]
  D -->|"notify(), notifyAll()"| B

  B -->|"sleep(), wait(timeout)"| E["TIMED_WAITING<br/>정해진 시간 동안 대기"]
  E -->|"시간 만료 또는 notify()"| B

  classDef start fill:#374151,stroke:#94a3b8,stroke-width:2px,color:#f8fafc
  classDef runnable fill:#166534,stroke:#22c55e,stroke-width:2px,color:#f8fafc
  classDef blocked fill:#991b1b,stroke:#f87171,stroke-width:2px,color:#f8fafc
  classDef waiting fill:#854d0e,stroke:#f59e0b,stroke-width:2px,color:#f8fafc
  classDef terminal fill:#4b5563,stroke:#94a3b8,stroke-width:2px,color:#f8fafc
