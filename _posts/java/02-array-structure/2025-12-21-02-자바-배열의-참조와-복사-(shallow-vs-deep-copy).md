...
> **Tip:** `int[]`, `double[]` 같은 1차원 기본형 배열에서는 `clone()`이 새로운 배열 인스턴스를 만들고 내부 값을 그대로 복사하므로 깊은 복사처럼 동작한다.
> 
> 하지만 `Person[]` 같은 객체 배열에서는 배열 인스턴스만 새로 만들어지고, 배열 안의 각 요소는 기존 객체의 참조값을 그대로 복사한다. 따라서 배열 자체는 분리되지만, 내부 객체는 공유된다.
>
> **2차원 이상의 다차원 배열**도 같은 원리다. 자바의 2차원 배열은 실제로는 `int[][]`라는 하나의 연속된 2차원 공간이 아니라, `int[]` 배열들을 참조하는 "배열의 배열" 구조다. 그래서 `clone()`을 호출하면 바깥 배열만 새로 만들어지고, 하위 배열에 대한 참조는 그대로 복사된다. 다차원 배열의 완전한 깊은 복사는 하위 배열까지 재귀적으로 복사하거나 다른 방식을 사용해야 한다.
{: .prompt-tip }
